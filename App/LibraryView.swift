    ...
    private func confirmation(for action: PendingAction) -> Alert {
        ...
        case .deleteDownload(let issue):
            return Alert(
                ...
                message: Text(DeleteCopy.removeDownloadMessage(
                    issueName(issue), bytes: model.downloadedSizes[issue.id])),
                primaryButton: .destructive(Text("Yes")) { model.deleteDownload(issue) },
                secondaryButton: .cancel(Text("No")))
        case .remove(let issue):
            return Alert(
                title: Text("Are you sure?"),
                message: Text(DeleteCopy.deleteMessage(issueName(issue),
                                                        synced: model.syncsWithICloud)),
        ...
        case .undeletable(let issue):
            ...
            guard issue.isDownloaded else {
                return Alert(title: Text("From the app index"),
                             message: Text(DeleteCopy.undeletableMessage(downloaded: false)),
                             dismissButton: .default(Text("OK")))
            }
            return Alert(
                title: Text("From the app index"),
                message: Text(DeleteCopy.undeletableMessage(downloaded: true)),
        ...
        case let .removeVisibleDownloads(count, touchesASet):
            return Alert(
                title: Text(DeleteCopy.removeVisibleTitle(count)),
                message: Text(DeleteCopy.removeVisibleMessage(count, touchesASet: touchesASet)),
                primaryButton: .destructive(Text("Yes")) { model.removeVisibleDownloads() },
                secondaryButton: .cancel(Text("No")))
        case let .deleteVisible(count, shipped):
            return Alert(
                title: Text(DeleteCopy.deleteVisibleTitle(count)),
                message: Text(DeleteCopy.deleteVisibleMessage(
        ...
        case let .deleteAll(count, shipped):
            ...
            return Alert(
                title: Text("Are you sure?"),
                message: Text(DeleteCopy.deleteAllMessage(count, shipped: shipped,
                                                           local: local.count,
                                                           synced: model.syncsWithICloud)),
        ...
        case .deleteLocalFile(let issue):
            return Alert(
                title: Text("Are you sure?"),
                message: Text(DeleteCopy.deleteLocalFileMessage(issueName(issue))),
        ...
        case let .deleteLocalFiles(count, bytes):
            return Alert(
                ...
                message: Text(DeleteCopy.deleteLocalFilesMessage(count, bytes: bytes)),
