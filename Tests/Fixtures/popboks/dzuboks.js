...
var bottomTile  = [256, 13];
var cornerTile  = [176, 13];
...
window.onload = function () {
    ...
    if (qmPos != -1) {
        ...
        try {
            ...
            if ((issue >= 1) && (issue <= 194) && (numbers[issue-1][1] > 0) && (page > 0) && (page <= numbers[issue-1][1])) {
                renderPage();
            } else {
                showYears();
            }
...
function showYears() {
    ...
    var underLogoTxt = document.createTextNode("Džuboks arhiva 1974-1985.");
    ...
    for (var i = 0; i < years.length; i++) {
        ...
        var yearTxt = document.createTextNode("19" + years[i][0] + ".");
...
function openYear(yearId) {
    ...
    var totalIssues = 0;
    for (var j = years[yearId-1][1][0]; j <= years[yearId-1][1][1]; j++) {
        if (numbers[j-1][1] != 0) totalIssues += 1;
    }
    ...
    for (var j = years[yearId-1][1][0]; j <= years[yearId-1][1][1]; j++) {
        if (numbers[j-1][1] == 0) {
            ...
            issueImg.src = "/dzuboks/dzuboksimages/" + j + "/1/icon.jpg";
            issueImg.id = "iss_" + j;
            ...
            var issueNoTxt = document.createTextNode(" " + j + " ");
...
function openissue(iss) {
    ...
    page = 1;
    // tile sizes override
    if (numbers[issue-1][2] !== undefined) {
        if (numbers[issue-1][2] == 2) {
            regularTile = [256, 256];
            rightTile   = [176, 256];
            bottomTile  = [256, 105];
            cornerTile  = [176, 105];
        } else if (numbers[issue-1][2] == 1) {
            regularTile = [256, 256];
            rightTile   = [176, 256];
            bottomTile  = [256, 154];
            cornerTile  = [176, 154];
...
function zoom(inout) {
    ...
    if (oldStatusSpan) {
        ...
        var newStatusTxt = document.createTextNode("Džuboks broj " + issue + ", strana " + page + " (" + zoomLevel + "%)");
...
function renderPage() {
    ...
    var statusTxt = document.createTextNode("Džuboks broj " + issue + ", strana " + page + " (" + zoomLevel + "%)");
    ...
    for (var i=1; i <= numbers[issue-1][1]; i++) {
    ...
    for (var v = 0; v <= 6; v++) {
        ...
        for (var h = 0; h <= 4; h++) {
            ...
            tileImg.src = "/dzuboks/dzuboksimages/" + issue + "/" + page + "/2_" + h + "_" + v + ".jpg";
